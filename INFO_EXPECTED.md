...
3. **`certifications`** — Same for `certifications` via `/details/certifications/`.  
4. **`skills`** — Same for `skills` via `/details/skills/`.  
5. **`languages`** — **Special case:** Base and deep language arrays are **union-merged** by normalized language key (`mergeLanguageLists`); deep can add or fill proficiency without dropping base-only rows when the merge grows the combined list.  
6. **`honors`** — Same replacement/merge pattern as other array sections via `/details/honors/`.  
7. **`publications`** — Same via `/details/publications/`.
...
| Item | Source |
|------|--------|
| Eleven export section toggles + footer note | `src/popup/popup.html`, `print-main.js` |
| Header default-on | `settings.profileHeader !== false` in `print-main.js` |
| Seven deep slugs and merge order | `DEEP_SECTION_SLUGS`, `plannedDeepSections` in `src/background.js` |
